...
namespace DotJson.Core.Parser
{
	...
	// 
	public sealed class DotJsonMiniTokenizer : LiteJsonTokenizer
	{
	...
	//    private charQueue charQueue = null;
		...
		// Ctor's
		public DotJsonMiniTokenizer(string str)
            : this(new StringReader(str))
		{
		}
		public DotJsonMiniTokenizer(TextReader reader)
            : this(reader, CHARQUEUE_SIZE)
		{
		}
		public DotJsonMiniTokenizer(TextReader reader, int charQueueSize)
		{
		...
		private JsonToken PrepareNextToken()
		{
			...
			default:
				if(Symbols.IsStartingNumber(ch)) {
					...
					throw new DotJsonMiniException("Invalid symbol encountered: ch = " + ch);
				}
				break;
			}
	...
	//        }
		...
		private Number ReadNumber()
		{
			...
			if(! Symbols.IsStartingNumber(c)) {
				throw new DotJsonMiniException("Expecting a number. Invalid symbol encountered: c = " + c);
			}
			...
			if(c == (char) CharSymbol.PERIOD) {
				...
				if(c == '0') {
					...
					if(Char.IsDigit(c2)) {
						throw new DotJsonMiniException("Invalid number: c = " + sb.ToString() + c + c2);
					}
					sb.Append(c);
                    if (c2 == (char) CharSymbol.PERIOD) {
						periodRead = true;
			...
			while(d != 0 && (Char.IsDigit(d) || 
				...
				if(Symbols.IsExponentChar(d)) {
					...
					if(d2 == (char) CharSymbol.PLUS || d2 == (char) CharSymbol.MINUS || Char.IsDigit(d2)) {
						...
						throw new DotJsonMiniException("Invalid number: " + sb.ToString() + d2);
					}
					exponentRead = true;
				}
				d = PeekChar();
			...
			try {
				...
				// ???
				throw new DotJsonMiniException("Invalid number encountered: str = " + str, e);
			}
			return number;
		}
